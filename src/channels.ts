...
export default function(app: Application): void {
  ...
  app.on("login", (authResult: any, { connection }: any): void => {
    ...
    if (connection) {
      ...
      // if(userSchema.isAdmin) { app.channel('admins').join(connection); }

      // If the userSchema has joined e.g. chat rooms
      // if(Array.isArray(userSchema.rooms)) userSchema.rooms.forEach(room => app.channel(`rooms/${room.id}`).join(connection));
